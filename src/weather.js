...
import { displayFData, displayCData } from './displayData';
...
export async function defaultWeather() {
  ...
  const responseTwo = await fetch(`https://api.weatherapi.com/v1/forecast.json?key=${API_KEY_TWO}&q=London`);
...
export async function getForecast(place) {
  const response = await fetch(`https://api.weatherapi.com/v1/forecast.json?key=${API_KEY_TWO}&q=${place}`);
